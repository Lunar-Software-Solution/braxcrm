...
export default function Objects() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const typesData = await listObjectTypes();
        ...
        {loading ? (
          ...
          <ScrollArea className="h-full">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => { setEditingType(type); setDialogOpen(true); }}>
                          <Edit className="h-3.5 w-3.5" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => { setTypeToDelete(type); setDeleteDialogOpen(true); }}>
                          <Trash2 className="h-3.5 w-3.5" />
                        </Button>
                      </div>
                    </div>
                    {type.description && <CardDescription className="line-clamp-2">{type.description}</CardDescription>}
                  </CardHeader>
                  <CardContent>
              ...
              <Card className="border-dashed cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setDialogOpen(true)}>
        ...
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSave(new FormData(e.currentTarget)); }} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="name" name="name" placeholder="e.g., Influencer, Supplier" defaultValue={editingType?.name} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="description">Description</Label>
              <Textarea id="description" name="description" placeholder="Brief description..." defaultValue={editingType?.description || ""} />
            </div>
            <div className="space-y-2">
              <Label>Color</Label>
              <div className="flex flex-wrap gap-2">
                {defaultColors.map((color) => (
                  <label key={color} className="cursor-pointer">
                    <input type="radio" name="color" value={color} defaultChecked={color === (editingType?.color || defaultColors[0])} className="sr-only" />
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete "{typeToDelete?.name}"? This will remove it from all associated people. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
